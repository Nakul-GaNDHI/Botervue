...
def main():
    ...
    job_description=st.text_area("Write the Job Description ", height=200)

    if st.button("Analyze Match") and uploaded_file and job_description: # type: ignore
...
#gauge chart
    ...
    if similarity_score<40:
        st.warning("Your resume has a low match with the job description. Consider revising it to include more relevant keywords.")
    elif similarity_score<70:
        st.info("Your resume has a moderate match. You might want to enhance it further.") 
    else:
        st.success("Great job! Your resume matches well with the job description.") 
